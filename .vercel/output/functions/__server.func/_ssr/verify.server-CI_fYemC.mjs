...
import { n as gateIdentityEnabled, t as auth } from "./server-BziVAaMl.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/verify.server-CI_fYemC.js
if (Boolean(process.env.DATABASE_URL?.trim()) && true) console.error("[auth] DATABASE_URL is set but auth is disabled (VITE_AUTH_ENABLED=false) — requireUserId() will reject every request (fail closed) rather than share one dev user on a real database.");
...
async function requireUserId(bearerToken) {
	if (!gateIdentityEnabled()) return DEV_USER_ID;
