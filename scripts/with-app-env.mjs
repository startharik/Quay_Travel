...
function main(argv) {
  ...
  const resolvedCommand = isWindows && !command.includes("/") && !command.includes("\\") && !command.toLowerCase().endsWith(".cmd") && !command.toLowerCase().endsWith(".exe")
    ? `${command}.cmd`
    : command;
  const child = spawn(resolvedCommand, args, { stdio: "inherit", env, shell: isWindows });
